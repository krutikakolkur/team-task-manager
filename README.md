...
Add an environment variable JWT_SECRET set to a long random string (Railway → Variables tab).
Once deployed, Railway gives you a public URL — that's your live app (frontend + API together, since Express serves both).
...
Replace better-sqlite3 with pg (or your driver of choice) in package.json.
Port the CREATE TABLE statements in db.js (they're already standard SQL with minor SQLite-specific syntax like AUTOINCREMENT/datetime('now') to adjust).
Swap db.prepare(...).get()/.all()/.run() calls in the route files for your driver's query methods.
🎬 Demo script (for your 2–5 min video)
Sign up as "Alice" → create a project → note you're the Admin.
Sign up as "Bob" in a second browser/incognito window.
As Alice, invite Bob to the project as a Member.
As Alice, create a task and assign it to Bob, with a due date.
As Bob, open the project, change the task status to "In Progress" then "Done".
As Bob, try to delete a task or add a member → show the 403 / role restriction.
Show the dashboard counts updating live.
